-- =============================================================================
-- Termine in der Vergangenheit: Hinweis statt Sperre (FIX-019, BEF-012)
--
-- Seit CAL-003 wies der Server jeden Tag vor dem heutigen ab - beim Anlegen
-- wie beim Aendern. Keine Leitplanke verlangt das; es war ein Schutz vor
-- Tippfehlern. Festlegung von Jannes (2026-09-18): Ein Termin darf in die
-- Vergangenheit gelegt werden, mit einem Hinweis an die bedienende Person.
--
-- Muster wie bei der Arbeitszeit (CAL-005): Ohne ausdrueckliche Bestaetigung
-- (`p_confirmed_past`) bleibt die Abweisung; mit ihr wird geschrieben, und
-- der Auditeintrag traegt `in_the_past`. So bleibt der Tippfehler-Schutz -
-- die Oberflaeche fragt nach -, und das Nachtragen wird moeglich (ANN-057).
--
-- Beide Funktionen bekommen dafuer einen Parameter mehr; PostgreSQL kennt
-- kein Ergaenzen einer Signatur, deshalb drop und neu. Die Koerper sind
-- unveraendert aus 20260917110000_free_appointment_length.sql bis auf die
-- Vergangenheitspruefung und den Auditkontext.
-- =============================================================================

drop function public.create_appointment(uuid, uuid, text, date, time, time, uuid, boolean, uuid);
drop function public.update_appointment(uuid, timestamptz, uuid, text, date, time, time, uuid, boolean);
...
comment on function public.create_appointment(uuid, uuid, text, date, time, time, uuid, boolean, uuid, boolean) is
...
revoke all on function public.create_appointment(uuid, uuid, text, date, time, time, uuid, boolean, uuid, boolean)
  from public, anon;
grant execute on function public.create_appointment(uuid, uuid, text, date, time, time, uuid, boolean, uuid, boolean)
  to authenticated;
...
comment on function public.update_appointment(uuid, timestamptz, uuid, text, date, time, time, uuid, boolean, boolean) is
  'Aendert einen bestaetigten Termin. Prueft eine geaenderte Laenge gegen das Praxisraster (CAL-020), einen geaenderten Beginn gegen das Raster und eine Neuplanung gegen die Arbeitszeit (CAL-003, CAL-005, CAL-015b, ADR-018). Ein Tag in der Vergangenheit verlangt p_confirmed_past und steht im Auditkontext (FIX-019, ANN-057).';

revoke all on function public.update_appointment(uuid, timestamptz, uuid, text, date, time, time, uuid, boolean, boolean)
  from public, anon;
grant execute on function public.update_appointment(uuid, timestamptz, uuid, text, date, time, time, uuid, boolean, boolean)
  to authenticated;
