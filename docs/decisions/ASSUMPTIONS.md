...
**Begründung.** Ein signierter Verweis entsteht im Browser, und die Datenbank sieht davon nichts; eine serverseitige Zwischenstelle gibt es nicht, weil eine Edge Function für produktive Gesundheitsdaten nach ADR-015 Punkt 20 nicht freigegeben ist. Die Auditpflicht aus ADR-017 Punkt 20 ist damit nicht erzwingbar, aber praktisch unumgehbar: Wer den Schlüssel nicht hat, kann keinen Verweis erzeugen, und der Schlüssel ist eine Kette aus drei zufälligen UUID (ADR-017 Punkt 5). Den Schlüssel mitzuliefern hieße, auf Wohlverhalten des Clients zu setzen; ganz auf den Eintrag zu verzichten verstößt gegen ADR-010 Punkt 2.

**Anker.** `supabase/migrations/20260913110000_patient_files.sql`: `list_patient_files` (ohne `object_key`), `issue_patient_file_link` (mit Auditeintrag), `app.may_read_patient_file_object`; `oeffneDatei` in `src/features/files/api.ts`; Tests in `supabase/tests/patient-files.test.ts`, Abschnitt „Auslieferung".

**Änderungspfad.** Fällt OPS-001 Punkt 5 positiv aus (Entzug eines Verweises ohne Support), ergänzt das die Annahme um ein Werkzeug · Aufwand `klein`. Wird die Edge Runtime freigegeben, kann die Ausstellung vollständig serverseitig laufen: eine Funktion, die selbst unterschreibt, und ein Lesepfad, der den Verweis statt des Schlüssels liefert · Aufwand `mittel`, ohne Änderung am Datenmodell.
