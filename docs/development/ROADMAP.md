...
Version 4.8 · Stand 2026-09-13 · **in Kraft**
...
- **Voraussetzung erfüllt: [ADR-017](../adr/ADR-017-file-storage.md) ist am
  ...
  bestätigt. Der Loop baut danach, ohne eigene Spielräume: zweiphasiger Upload
  mit serverseitiger Bestätigung, unveränderliche Dateien, Dokumentart als
  Rollenschnitt (**der Verordnungsscan ist klinisch**, für `office` nicht
  sichtbar), signierte Verweise mit **60 Sekunden** und `cacheControl: '0'`,
  drei Auditereignisse, Löschauftrag mit Quittung, **keine Virenprüfung in
  V1**. Dazu gehört die Nachbildung von `storage.objects` im Test-Shim —
  sonst bliebe der Dateizugriff (§12) im wichtigsten Gate der Cloudumgebung
  ungeprüft.
- **Was DAT-EPIC-001 nicht löst:** Die **produktive** Ablage hängt zusätzlich
  ...
  Der Loop selbst läuft gegen die lokale Umgebung und ist davon nicht
  blockiert.
...
| G4  | **DAT-EPIC-001 Dateiablage**                          | DAT-001 Bucket, Berechtigungen, signierte Verweise, Datenklasse, Audit — **alles nach ADR-017**, einschließlich zweiphasigem Upload mit Bestätigung, Dokumentart als Rollenschnitt, Löschauftrag mit Quittung und einer Nachbildung von `storage.objects` im Test-Shim · VER-004 Scan-Anhang je Verordnung                                                                                                                               | Claude                                | Nov 2026                |
