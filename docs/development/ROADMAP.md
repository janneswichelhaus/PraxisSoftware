...
- **Voraussetzung: [ADR-017](../adr/ADR-017-file-storage.md) liegt seit dem
  2026-09-12 vor** (Docs-Session zu Zeile G1) und wartet auf Jannes'
  Bestätigung — **acht Fragen** am Ende des ADR, je eine Zeile genügt. Die
  wichtigsten: signierte Verweise mit 60 Sekunden und **kein Teilen-Link**
  (weil ein solcher Verweis nicht widerrufbar ist), der **Verordnungsscan ist
  klinisch** und damit für `office` nicht sichtbar, und **keine Virenprüfung in
  V1**. **Antwortet Jannes nicht vor dem Loop**, gilt `PROJECT_PRINCIPLES.md`
  §15.1: Die Dateiablage ist kein Hard-Stop-Thema, der Loop folgt dem ADR in
  seiner vorgeschlagenen Fassung. Wer trotzdem lieber etwas anderes vorzieht,
  nimmt `ABR-EPIC-001` (Voraussetzung: B4 als Annahme).
...
- **Parallel als Docs-Sessions** (kein Code): im September bleibt `OPS-001
  Providerprüfung` (Dokument). `ADR-017 Dateiablage` ist am 2026-09-12
  geschrieben und wartet auf die Bestätigung; `ADR-019 Kartendienst` liegt seit
  dem 2026-09-08 in Fassung 2 vor (MAP-001) und wartet auf E-20. Jede endet mit
  einer Bestätigung durch Jannes.
...
| --- | ----------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------- | ----------------------- |
| G1  | **ADR-017 Dateiablage** — *geschrieben am 2026-09-12, Bestätigung offen (acht Fragen im ADR)* | Supabase Storage nach ADR-015 Punkt 10, signierte Verweise, Datenklasse und Retention nach ADR-008, Zugriff nach ADR-004, Audit nach ADR-010, Virenprüfung erst bei Patienten-Uploads. Schließt E8. **Die Reihenfolge zu OPS-001 ist präzisiert:** Storage ist kein neuer Dienstleister, deshalb ist der ADR vor der Prüfung schreibbar und liefert ihr **fünf zusätzliche Prüfpunkte**; **produktiv** wird die Ablage erst nach positivem OPS-001 und einem dokumentierten Sicherungsweg für den Bucket (G7). | Claude (Docs), Jannes bestätigt       | Sep 2026                |
...
| ---------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------- | ---------------------------- | ----------------------------------- | --------------------- |
| **E8 → ADR-017**             | Dateiablage: Ort, Zugriff, signierte Verweise, Retention, Virenprüfung                                                                             | Claude schreibt, Jannes bestätigt   | DAT-EPIC-001, ABR-EPIC-002b  | Sep 2026                            | **ADR-017 geschrieben 2026-09-12**, acht Fragen offen |
