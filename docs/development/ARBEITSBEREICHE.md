...
| Mitteilungsvermerk am Termin            | `/termine/:id` (Auswahl), Akte „Nächste Termine" (Zeichen) | Hält fest, ob und auf welchem Weg ein Termin mitgeteilt wurde: persönlich, telefonisch, Terminzettel, E-Mail. **Die Anwendung verschickt nichts** — der Vermerk beschreibt einen Vorgang außerhalb (B15, ANN-040). Er verfällt automatisch, sobald sich der Termin ändert; der Druck des Terminzettels vermerkt alle aufgeführten Termine (CAL-012) |
| Terminzettel zum Ausdrucken             | `/patienten/:id/terminzettel` (aus dem Abschnitt der nächsten Termine) | Datum, Zeit, Ort und behandelnde Person der nächsten bestätigten Termine; kein Status, keine Verordnung, keine Adresse. **Nur Druck, kein Versand** — B15 ist offen (CAL-011, `IDEA-PRX-006`, ANN-039). Der Aufruf wird als Aktenzugriff protokolliert |
