...
| Dateien in der Akte                     | `/patienten/:id/dateien`                                | Alles, was als Blatt vorliegt (DAT-001, ADR-017). **Die Dokumentart ist der Rollenschnitt:** klinisch — Befund, Arztbrief, Verordnungsscan, klinisches Bild — sehen `owner`, `therapist`, `team_lead`; organisatorisch — Einwilligung, Vertrag — sieht zusätzlich `office`. Was eine Rolle nicht sehen darf, liefert `list_patient_files` gar nicht erst: nicht ausgegraut und nicht gezählt. Hochgeladen wird zweiphasig mit serverseitiger Bestätigung gegen `storage.objects`; Dateien sind unveränderlich, eine Korrektur ist eine neue Datei. Ausgeliefert wird nur über einen signierten Verweis mit **60 Sekunden**, je Zugriff neu, erzeugt auf ausdrückliche Aktion und dabei protokolliert (`patient_file.link_issued`). Eine Datei ohne Objekt steht als Fehler in der Liste, nicht als leere Fläche. Kein Teilen-Link, keine Virenprüfung in V1, kein Upload durch Patient:innen. **Löschen ist zweistufig** (DAT-002): Die Zeile fällt sofort, für das Objekt entsteht ein Löschauftrag, den `owner` unter „Aufbewahrung und Löschung“ ausführt — quittiert wird erst, wenn der Server selbst feststellt, dass das Objekt weg ist. Die **Dokumentart korrigieren** dürfen nur die therapeutischen Rollen, und es ist ein protokollierter Vorgang: Er verschiebt eine Sichtbarkeitsgrenze. **Der Abgleich** (DAT-003) steht unter „Aufbewahrung und Löschung“: eine Datei ohne Objekt wird als Verlust mit Akte und Namen genannt und an der Datei selbst als Fehler angezeigt; ein Objekt ohne Datei wird gezählt und über einen gewöhnlichen Löschauftrag entfernt. Gerechnet wird beim Öffnen der Seite, nicht laufend — die Anwendung verschickt nichts |
