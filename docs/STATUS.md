# Status · Stand 2026-09-21 · letzte Session: Trainingsbereich zugeschnitten, Termine raus
...
**E18 ist vollständig** — Schritt 7 hat den Trainingsbereich geschnitten: **Etappe TR**, vier Loops
(TRN-EPIC-001 bis -004) vor der Eröffnung, die fünfzehn Navigationspunkte einzeln zugeordnet, die
drei MDR-nahen zuletzt; Etappe L hat das Fundament gebaut, eine Tür dorthin nicht. **Dazu sind die
Termine raus** (Roadmap 5.37): neun Blöcke statt elf Monatszeilen, Meilensteine als Zustände, ein
einziger Termin — **Eröffnung Juli 2027**. **41,1 %.**
...
- **Lokal:** `git pull`; `pnpm install` nur, falls MAP-002 noch nicht gezogen ist (`maplibre-gl`). **Keine Migration, kein `db reset`.** **Node 22** (`.nvmrc`), sonst rot.
...
- **B8:** Lizenzbeleg · **OPS-001** offen · ungemergt liegen `claude/issue-42-status-fv319v` (3), `claude/r3-analyse` (5) und `claude/r3-code-review-hardening-c22b8f` (1 Commit).
...
**Der Trainingsbereich hat einen Plan, aber noch keine Erlaubnis** — Etappe TR steht, die Freigabe
nach §14 liegt bei Jannes. **Und der Plan hat keinen Kalender mehr:** Reihenfolge statt Monate,
kein Takt, kein Puffer in Wochen. Lokal: `git pull origin main`, sonst nichts.
