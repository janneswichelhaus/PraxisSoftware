# Status · Stand 2026-09-22 · letzte Session: Roadmap 6.0
...
**Roadmap 6.0 ist in Kraft.** Zur Eröffnung im Juli 2027 läuft das Endprodukt: Kern, Tagesroute, Befund, Praxisverwaltung, Training, Plattform für Patient:innen und Kund:innen mit Abo und Paketen, Praxisbetrieb, KI-Assistenz (`PROJECT_PRINCIPLES.md` 0.16 §14). Externe Prüfungen (OPS-001, B1, B2, B4) kommen **nach** dem Feature-Freeze M2. Was §17 verbietet, steht unter „Nicht in V1". Fortschritt **32,7 %** (vorher 48,9 — der Nenner enthält jetzt das Endprodukt).

## Danach — Block 1, dann Block 2

1. **G19 Dokumentationsgate erweitern (BEF-028)** — `/feature-loop G19 Dokumentationsgate erweitern nach docs/development/ROADMAP.md, Block 1`
2. **G6a Abgewiesene Zugriffe, Rest** — vor Training und Plattform, die neue Abweisungspfade bringen
3. **MAP-006 Tagesroute** mit Fahrpuffer (`docs/development/MAP-LOOPS.md`) — synthetische Adressen; das Gate steht vor dem Scharfschalten

Danach FRB-EPIC-001 → FRB-EPIC-002 → FRB-EPIC-003 → DOK-005 → PRX-EPIC-001 bis 003. **Docs-Session ADR-023 Plattformzugang** darf jederzeit dazwischen laufen und muss vor Block 4 angenommen sein.

## Prüfverfahren

**Die CI läuft.** Lokal: `pnpm test:db` gegen einen Wegwerf-Container (54329); **angemeldete E2E-Tests und die Deno-Laufzeit laufen in der Cloud nicht**. Stand heute: `test` und `test:db` unverändert (keine Migration, keine Policy); geändert sind Dokumente, `fortschritt.json` und drei Verweistexte in `src/app/mdr.ts`. Keine Sichtprüfung: keine Oberfläche geändert.
...
**Roadmap 6.0, Planungssession ohne Code.** Jannes hat den Umfang auf das Endprodukt erweitert, die Prüfungen hinter den Freeze gelegt und die Reihenfolge delegiert. Neu: `PROJECT_PRINCIPLES.md` 0.16 (§14 gibt Trainingsbereich, Plattform und Portal-Abo frei), B9/B11/B15 neu entschieden, Chronik in `ROADMAP-CHRONIK.md`. **Lokale Schritte:** `git pull origin claude/beautiful-maxwell-rno44c` — keine Migration, keine neue Abhängigkeit.
