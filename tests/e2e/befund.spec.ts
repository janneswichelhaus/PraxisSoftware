...
test.describe('Befund', () => {
  ...
  test('waehlt einen Bereich durch Antippen der Figur', async ({ page }) => {
    await page.setViewportSize({ width: 375, height: 800 });
    await page.goto(PRUEFSEITE);
    const knie = page.locator('fieldset [data-bereich="knie_links"]');
    await knie.scrollIntoViewIfNeeded();
    await knie.click();
    await expect(page.getByText(/Gewählt: .*Knie links/)).toBeVisible();
  });
